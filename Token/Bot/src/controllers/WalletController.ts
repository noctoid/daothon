...
//            await replyWithMarkdown("格式不对，请输入 `/transfer [Matataki UID/@Telegram 用户名] [symbol] [amount]`");
              await replyWithMarkdown("Wrong format，please input: `/transfer [Matataki UID/@Telegram 用户名] [symbol] [amount]`");
            return;
        }
...
Sender：[${senderInfo.user.name}](${this.matatakiService.urlPrefix}/user/${senderInfo.user.id})
Reciever：[${targetName}](${this.matatakiService.urlPrefix}/user/${userId})
Amount：${amount / 10000} ${symbol}`;
        const transactionMessage = await replyWithMarkdown("Transaction Pending..." + commonMessage, { disable_web_page_preview: true });

        let finalMessage, replyMarkup;
        try {
            const tx_hash = await this.matatakiService.transfer(senderInfo.user.id, userId, symbol, amount);

            finalMessage = "Transaction Success" + commonMessage;

            replyMarkup = Markup.inlineKeyboard([
                [Markup.urlButton("Transaction Detail:", `https://rinkeby.etherscan.io/tx/${tx_hash}`)]
            ]);
        } catch {
            finalMessage = "Transaction Fail" + commonMessage;
        }
